...
class RAFLevel4SignatureTask(RemoteSimulationTask):
    ...
    def _build_remote_environment(self, server_url: str) -> TaskEnvironment:
        ...
        api = RAFSignatureAPI(description=self._instructions, dataset=dataset, client=client)
    ...
    def _prepare_dataset(
        ...
        """Pre-compute simulator runs for each saturation/seed pair."""
        dataset: dict[str, Any] = {
            "saturations": [str(t) for t in self._saturations],
            "snapshot_times": list(self._snapshot_times),
            "simulators": {str(t): [] for t in self._saturations},
        }
        ...
        for sat in self._saturations:
            ...
            for seed in self._train_seeds:
                ...
                dataset["simulators"][sat_key].append(
                    {
                        "macro_seed": seed,
